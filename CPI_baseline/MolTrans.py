...
class MolTrans():
    ...
    def train(self, args, logger, train_loader, val_loader=None):
        for epoch in range(self.NUM_EPOCHS):
            self.model.train()
            for batch_idx, (d, p, d_mask, p_mask, label) in enumerate(train_loader):
    ...
    def predict(self, loader):
        ...
        with torch.no_grad():
            for batch_idx, (d, p, d_mask, p_mask, label) in enumerate(loader):
                d = d.to(self.device)
                p = p.to(self.device)
    ...
    def save_model(self, path):
        torch.save(self.model.state_dict(), os.path.join(path, 'MolTrans.pt'))
